package org.example.vitalance.security.config;

import org.springframework.web.filter.CorsFilter;
import org.example.vitalance.security.filters.JwtRequestFilter;
import org.example.vitalance.security.services.CustomUserDetailsService;
...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers(
                                "/api/authenticate",//login y obtener JWT
                                "/apiUser/registrar",
                                "/swagger-ui/**", //cargar la UI
                                "/v3/api-docs/**", //Leer endpoints y documentarlos
                                "/swagger-resources/**",//depedencia interna
                                "/webjars/**"//depedencia interna
                        ).permitAll()
    ...
    public CorsFilter corsFilter() {
        ...
        config.addAllowedOrigin("${ip.frontend}"); //usa el valor de application.properties
        config.addAllowedMethod("*"); //permite todos los metodos(get,post,put,..)
        config.addExposedHeader("Authorization");  //expone el token en la respuesta
        config.addAllowedHeader("*"); //permite todos los headers (Authorization, Content-Type, etc.)
        source.registerCorsConfiguration("/**", config); //para todos los paths ,endpoints
